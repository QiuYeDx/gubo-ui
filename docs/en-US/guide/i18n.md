...
GuBo UI components use English **by default**. If you want to use other
languages, read on to find out how.

## Global configuration

GuBo UI provides global configuration options.
...
GuBo UI also provides a Vue component [ConfigProvider](../component/config-provider.md)
for globally configuring locale and other settings.

```vue [App.vue]
<template>
  <g-config-provider :locale="zhCn">
    <app />
  </g-config-provider>
</template>
...
If you are using GuBo UI via CDN, you need to do the following. Let's take
unpkg as an example:
