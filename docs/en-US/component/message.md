...
:::demo The setup of Message is very similar to notification, so parts of the options won't be explained in detail here. You can check the options table below combined with notification doc to understand it. GuBo UI has registered a `$message` method for invoking. Message can take a string or a VNode as parameter, and it will be shown as the main body.
...
GuBo UI has added a global method `$message` for `app.config.globalProperties`. So in a vue instance you can call `Message` like what we did in this page.
...
In this case you should call `GMessage(options)`. We have also registered methods for different types, e.g. `GMessage.success(options)`. You can call `GMessage.closeAll()` to manually close all the instances.
...
If you globally registered GMessage component, it will automatically inherit your app context.
