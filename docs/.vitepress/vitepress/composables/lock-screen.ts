...
export const useLockScreen = () => {
  ...
  const cleanup = () => {
    if (!isClient) return
    removeClass(document.body, 'g-popup-parent--hidden')
  ...
  const lock = () => {
    if (!isClient) return
    withoutHiddenClass = !hasClass(document.body, 'g-popup-parent--hidden')
    if (withoutHiddenClass) {
    ...
    addClass(document.body, 'g-popup-parent--hidden')
  }
